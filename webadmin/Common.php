...
//Initialize Common Variables @0-F271FC83
session_start();
header('Pragma: ');
...
define("TemplatePath", "./");
define("ServerURL", "http://localhost/skybroadcast/webadmin/");
//define("ServerURL", "http://www.skybroadcast.net/webadmin/");
define("SecureURL", "");
...
//skybroadcast Connection Class @-5FCD62CB
class clsDBskybroadcast extends DB_MySQL
{
    ...
    function Initialize()
    {
        ...
        $this->PageSize = 0;
        $this->DB = "MySQL";
        $this->DBDatabase = "skybroadcast";
        $this->DBHost = "localhost";
        //$this->DBHost = "82.146.34.6";
        $this->DBUser = "skybroadcast";
        $this->DBPassword = "skybroadcast";
...
//End skybroadcast Connection Class
...
//CCConvertDataArrays @0-38F18A70
...
//CCLoginUser @0-646E6427
function CCLoginUser($login, $password)
{
    $db = new clsDBskybroadcast();
    $SQL = "SELECT login, login FROM users WHERE login=" . $db->ToSQL($login, ccsText) . " AND password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    ...
    if($Result)
    {
        ...
        CCSetSession("GroupID", $db->f("login"));
    }
    $db->close();
    return $Result;
}
